...
// Sert de proxy entre le frontend (Pages) et Cloudflare Workers AI (gratuit,
// inclus dans le compte Cloudflare — aucune clé API à gérer), et donne accès
// à D1 (env.DB) pour les futures routes de synchronisation.
//
// Déploiement :
//   npx wrangler deploy
//
...
async function handleAI(request: Request, env: Env, cors: HeadersInit): Promise<Response> {
  ...
  let answer: string
  try {
    // Modèle gratuit inclus dans le quota Workers AI. Catalogue complet :
